...
public class Input {
	...
	public static void update() {
		mousePos.x = MouseInfo.getPointerInfo().getLocation().x - Game.instance.screen.getLocationOnScreen().x;
		mousePos.y = MouseInfo.getPointerInfo().getLocation().y - Game.instance.screen.getLocationOnScreen().y;
		...
		if (check(Keys.A)) {
			Logger.Log("Key A is down.");
		}
		if (pressed(Keys.S)) {
			Logger.Log("Key S is pressed.");
		}
		if (released(Keys.D)) {
			Logger.Log("Key D is released.");
		}
